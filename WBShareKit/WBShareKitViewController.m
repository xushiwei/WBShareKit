...
- (IBAction)StartSinaPhotoWeibo:(id)sender {
    ...
    [[WBShareKit mainShare] sendSinaPhotoWithStatus:@"WBShareKit Photo test" lat:0 lng:0 path:[[NSBundle mainBundle] pathForResource:@"three" ofType:@"png"] delegate:self successSelector:@selector(sendRecordTicket:finishedWithData:) failSelector:@selector(sendRecordTicket:failedWithError:)];
}
...
- (IBAction)StartSendDoubanShuo:(id)sender {
    [[WBShareKit mainShare] sendDoubanShuoWithStatus:@"你妹啊啊啊阿阿阿阿啊" delegate:self successSelector:@selector(sendDoubanShuoTicket:finishedWithData:) failSelector:@selector(sendDoubanShuoTicket:failedWithError:)];
}
